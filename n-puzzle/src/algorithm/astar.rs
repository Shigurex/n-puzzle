...
pub(super) fn solve(puzzle: &Puzzle, heuristic: Heuristic, timeout: Option<u64>) -> Result<Output> {
    match heuristic {
        Heuristic::Manhattan => astar(puzzle.clone(), manhattan, false, timeout),
        Heuristic::Hamming => astar(puzzle.clone(), hamming, false, timeout),
        Heuristic::LinearConflict => astar(puzzle.clone(), linear_conflict, false, timeout),
